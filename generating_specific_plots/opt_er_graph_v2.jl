...
function calc_er_graph(;samps=1000)
	...
	for p in ps	
		...
		for n in collect(4:1:max_n)
			...
			for q in collect(1:1:samps)
				...
				if abs(val3) <= 10^(-5)
					avg_SOC_dev+= 1/(samps)
					avg_SOC_P1_dev+= 1/(samps)
					avg_swap_dev+= 1/(samps)
				else
					avg_SOC_dev+= abs(val1)/(abs(val3)*samps)
					avg_SOC_P1_dev+= abs(val2)/(abs(val3)*samps)
					avg_swap_dev+= abs(val4)/(abs(val3)*samps)
				end				
	...
	println(data)
	FileIO.save("data_er_graph_v2.jld2","data",data)
	FileIO.save("copy_data_er_graph_v2.jld2","data",data)
	#Plots.savefig(pl,"/ascldap/users/kevthom/quantum-max-cut/code/figs/opt_ER_approx_v4.pdf")	
